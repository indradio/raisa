                ...
                <div class="modal-body">
                        ...
                        <div class="col-md-12 align-content-start">
                            ...
                            <div class="form-group">
                                ...
                                <select class="selectpicker" name="durasi_aktivitas" id="durasi_aktivitas" data-style="select-with-transition" data-width="auto" required>
                                <?php
        ...
        <div class="modal-content">
            <div class="card card-signup card-plain">
                <form class="form" method="post" action="<?= base_url('lembur/tambah_aktivitas'); ?>">
                    <div class="modal-body">
                        <!-- <input type="text" class="form-control" id="id_lembur_direct" name="id_lembur_direct" /> -->
                        <div class="col-md-12 align-content-start">
                            <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('#dtaktivitas').DataTable({
            ...
            processing: true,
            ajax: {
                    "url"   : "<?= site_url('lembur/get_aktivitas/rencana') ?>",
                    "type"  : "POST",
                    "data"  : {id:$('#id').val()}
                },
            columns: [
        ...
        $('#tambahAktivitas').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body input[name="id_lembur"]').val(id);
        });
        ...
        $('#btn_tambah_aktivitas').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?= site_url('lembur/aktivitas/rencana/tambah') ?>",
                dataType : "JSON",
                data : {id:$('#id_lembur').val(), aktivitas:$('#aktivitas').val(), durasi:$('#durasi_aktivitas').val()},
        ...
        $('#btn_tambah_aktivitas_direct').on('click',function(){
            $.ajax({
                ...
                url  : "<?= site_url('lembur/aktivitas/rencana/tambah') ?>",
                dataType : "JSON",
                data : {id:$('#id').val(), kategori:$('#kategori').val(), aktivitas_direct:$('#aktivitas_direct').val(), aktivitas_projek:$('#aktivitas_projek').val(), copro:$('#copro').val(), durasi:$('#durasi_aktivitas_direct').val()},
                success: function(result){
        ...
        $('#btn_hapus_aktivitas').on('click',function(){
            $.ajax({
                ...
                dataType : "JSON",
                data : {id:$('#id_lembur_hapus').val(), id_aktivitas:$('#id_aktivitas').val()},
                success: function(result){
        ...
        $('#btn_ubah_tglmulai').on('click',function(){
            $.ajax({
                type : "POST",
                url  : "<?= site_url('lembur/ubah_jam/rencana') ?>",
                dataType : "JSON",
